...
public class DeleteApplicantBootcampContentCommand
    : IRequest<DeletedApplicantBootcampContentResponse>,
        ISecuredRequest,
        ...
        ITransactionalRequest
{
    ...
    public class DeleteApplicantBootcampContentCommandHandler
        : IRequestHandler<DeleteApplicantBootcampContentCommand, DeletedApplicantBootcampContentResponse>
    {
        ...
        public DeleteApplicantBootcampContentCommandHandler(
            IMapper mapper,
        ...
        public async Task<DeletedApplicantBootcampContentResponse> Handle(
            DeleteApplicantBootcampContentCommand request,
            CancellationToken cancellationToken
        )
        {
            ApplicantBootcampContent? applicantBootcampContent = await _applicantBootcampContentRepository.GetAsync(
                predicate: abc => abc.Id == request.Id,
                cancellationToken: cancellationToken
            );
            await _applicantBootcampContentBusinessRules.ApplicantBootcampContentShouldExistWhenSelected(
                applicantBootcampContent
            );

            await _applicantBootcampContentRepository.DeleteAsync(applicantBootcampContent!, true);

            DeletedApplicantBootcampContentResponse response = _mapper.Map<DeletedApplicantBootcampContentResponse>(
                applicantBootcampContent
            );
            return response;
        }
