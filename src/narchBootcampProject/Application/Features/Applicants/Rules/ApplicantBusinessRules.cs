...
public class ApplicantBusinessRules : BaseBusinessRules
{
    ...
    {
        var isExists = await _applicantRepository.GetAsync(applicant =>
            applicant.UserName == userName || applicant.Email == email
        );
        if (isExists is not null)
            throw new BusinessException(ApplicantsBusinessMessages.ApplicantExists);
    }
}
